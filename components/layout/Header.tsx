      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center gap-2">
            <span className="text-2xl font-display font-bold text-[#1A1614]">
            ...
            {navLinks.map(link => (
              <Link
                ...
                className={cn(
                  ...
                  pathname === link.href
                    ? 'bg-[#F5A623]/10 text-[#D4881A]'
                    : 'text-[#9C9189] hover:text-[#1A1614] hover:bg-[#F7F5F2]'
                )}
              >
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              ...
              <Link
                href="/auth/login"
                className="hidden sm:inline-flex bg-[#F5A623] text-[#1A1614] font-semibold text-sm px-4 py-2 rounded-[8px] hover:bg-[#D4881A] transition-colors"
              >
                {t('sign_in')}
              </Link>
            )}
            ...
            {/* Mobile menu toggle */}
            <button
              className="md:hidden p-2 text-[#9C9189] hover:text-[#1A1614]"
          ...
          {navLinks.map(link => (
            <Link
              ...
              className={cn(
                ...
                pathname === link.href
                  ? 'bg-[#F5A623]/10 text-[#D4881A]'
                  : 'text-[#1A1614] hover:bg-[#F7F5F2]'
              )}
            >
              {t(link.key as 'home' | 'search' | 'bookings')}
            </Link>
          ))}
          <div className="pt-2 border-t border-[#E0DDD8] flex items-center justify-between">
            <LangSwitch />
            {!user && (
              <Link
                href="/auth/login"
                onClick={() => setMobileOpen(false)}
                className="bg-[#F5A623] text-[#1A1614] font-semibold text-sm px-4 py-2 rounded-[8px] hover:bg-[#D4881A] transition-colors"
              >
                {t('sign_in')}
              </Link>
            )}
