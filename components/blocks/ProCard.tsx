...
export function ProCard({ pro, compact = false, className }: ProCardProps) {
  ...
  return (
    <Link href={`/pro/${pro.id}`}>
      <div className={cn(
        'bg-white rounded-[16px] p-4 shadow-sm border border-[#E0DDD8] hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer',
        compact ? 'flex gap-3 items-start' : 'flex flex-col gap-3',
        className
      )}>
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          {service && (
            <div className="text-sm">
              ...
              <span className="text-[#9C9189] text-xs">
          ...
          <div className="flex items-center gap-2">
            {pro.response_time && (
              <div className="flex items-center gap-1 text-xs text-[#9C9189]">
                <Clock size={11} />
                <span>{pro.response_time} {t('minutes')}</span>
              </div>
            )}
